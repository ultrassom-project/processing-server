...
public class CGNR: ReconstructionAlgorithm {
    ...
    public override func run(reconstructionInput: ReconstructionInput, errorConvergence: Float) -> ReconstructionOutput? {
        ...
        var f: [Float] = Array(repeating: 0.0, count: modelMatrixCols)
        var r: [Float] = reconstructionInput.signalVector
        var z: [Float] = BLAS.multiplyMatrixByVector(transposeM: true, alpha: 1, M: modelMatrix, MRows: modelMatrixRows, MCols: modelMatrixCols, A: r)
        var p: [Float] = z
        ...
        while error > errorConvergence {
            let w: [Float] = BLAS.multiplyMatrixByVector(transposeM: false, alpha: 1, M: modelMatrix, MRows: modelMatrixRows, MCols: modelMatrixCols, A: p)
            ...
            let zi = BLAS.multiplyMatrixByVector(transposeM: true, alpha: 1, M: modelMatrix, MRows: modelMatrixRows, MCols: modelMatrixCols, A: ri)
